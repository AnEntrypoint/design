...
export function partsFromMessages(assistantAndToolMessages) {
    ...
    for (const m of assistantAndToolMessages) {
        if (m.role === 'assistant') {
            ...
            let isError = false;
            // Tool results are JSON for structural error shapes (unknown tool,
            // tool unavailable, thrown exceptions) but YAML for a normal
            // handler's successful object return (freddie's dispatchTool
            // yaml.dumps those) -- check both shapes for an error field
            // instead of silently rendering a YAML error as done. No YAML
            // parser is bundled here (browser SDK, no js-yaml dep), so the
            // YAML case is a targeted top-level-key match, not a full parse.
            try { const parsed = JSON.parse(content); if (parsed && parsed.error) isError = true; }
            catch { if (typeof content === 'string' && /^error:\s*\S/m.test(content)) isError = true; }

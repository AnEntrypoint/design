...
function Bar() {
    return h('header', { style: 'padding:14px 32px;display:flex;justify-content:space-between;gap:16px;align-items:baseline' },
        h('a', { href: '#', class: 't-label', style: 'color:var(--paper);text-decoration:none' }, '← 247420'),
        h('span', { class: 't-micro', style: 'flex:1;align-self:center;margin:0 16px' }),
    ...
    return h('section', { style: 'padding:32px' },
        ...
        h('div', { style: 'font-family:var(--ff-mono);font-size:14px;max-width:560px' },
            ...rows.map(([k, v], i) => h('div', { key: i, style: 'display:flex;gap:12px;padding:6px 0;color:var(--paper)' },
                ...
                h('span', { style: 'flex:1;align-self:center' }),
                h('span', {}, v)
            ))
        )
    );
...
function Install({ cmd }) {
    return h('section', { style: 'padding:32px' },
        ...
        h('div', { style: 'display:flex;align-items:stretch;max-width:720px' },
            h('span', { style: 'font-family:var(--ff-mono);padding:14px 16px;color:#A9A396' }, '$'),
            h('code', { style: 'flex:1;font-family:var(--ff-mono);font-size:16px;padding:14px 16px;color:var(--paper);background:transparent' }, cmd),
            h('button', {
                onclick: () => { navigator.clipboard?.writeText(cmd); state.copied = true; render(); setTimeout(() => { state.copied = false; render(); }, 1200); },
                style: `font-family:var(--ff-mono);font-size:12px;letter-spacing:0.14em;text-transform:uppercase;padding:0 20px;background:${state.copied?'var(--acid)':'transparent'};color:${state.copied?'var(--acid-ink)':'var(--paper)'};cursor:pointer`
...
function Changelog({ entries }) {
    return h('section', { style: 'padding:32px' },
        ...
        h('div', {}, ...entries.map((e, i) =>
            h('div', { key: i, style: 'display:grid;grid-template-columns:100px 80px 1fr;gap:24px;padding:12px 0;color:var(--paper);align-items:baseline' },
...
function App() {
    return [
        ...
        h('footer', { style: 'padding:32px;display:flex;justify-content:space-between;gap:16px', class: 't-micro' },
